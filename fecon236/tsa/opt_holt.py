...
'''
_______________|  opt_holt.py :: optimize Holt-Winters parameters/forecast
...
CHANGE LOG  For latest version, see https://github.com/rsvp/fecon236
2018-05-31  fecon236 fork. Pass flake8. Fix imports, but
                PREPARE to be ABSORBED by holtwinters module:
                "ts." not needed for holt() and holtforecast()
                on lines 67, 131, and 133.
...
#  from fecon235.lib import yi_timeseries as ts   # ___ATTN___ Absorbtion.
...
def optimize_holt(dataframe, grids=50, alphas=(0.0, 1.0), betas=(0.0, 1.0)):
    ...
    losspc = (float(loss) / abs(tool.tailvalue(dataframe))) * 100
